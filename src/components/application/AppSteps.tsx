...
export default function AppSteps({ order }: {order: Order | null}) {
    ...
    const steps = [
        { id: "01", name: 'Create artwork', href: `/craft/image${(order_id !== null) ? `?order_id=${order_id}`: ""}`, status: segment === 'image' ? 'current' : 'complete', disabled: false },
        { id: "02", name: 'Insert message', href: `/craft/message${(order_id !== null) ? `?order_id=${order_id}`: ""}`, status: segment === 'image' ? 'upcoming' : segment === 'message' ? 'current' : 'complete', disabled: (!order || !order.image_url) },
        { id: "03", name: 'Add recipient', href: `/craft/recipient${(order_id !== null) ? `?order_id=${order_id}`: ""}`, status: segment === 'recipient' ? 'current' : 'upcoming', disabled: (!order || !order.top_message || !order.middle_message || !order.bottom_message) },
    ]

    return (
      <nav aria-label="Progress" className="mb-4 bg-gray-50 bg-opacity-20">
      <ol role="list" className="divide-y divide-gray-400 rounded-md border border-gray-400 md:flex md:divide-y-0">
